...
class GUI(tk.Tk):
    def __init__(self, controller: Controller):
        ...
        # Create circular Enroll button using Canvas
        self.enroll_canvas = tk.Canvas(self.button_frame, width=80, height=80, 
                                      highlightthickness=0)
        self.enroll_canvas.grid(row=0, column=1, padx=(5, 5), pady=(5, 5))
        
        # Draw circle button
        self.enroll_circle = self.enroll_canvas.create_oval(10, 10, 70, 70, 
                                                           fill='#0078d4', outline='#005a9e', width=2)
        self.enroll_text = self.enroll_canvas.create_text(40, 40, text="Enroll", 
                                                         fill='white', font=('Helvetica', 12, 'bold'))
        
        # Bind events for hover and click
        self.enroll_canvas.tag_bind(self.enroll_circle, '<Button-1>', lambda e: self.enroll())
        self.enroll_canvas.tag_bind(self.enroll_text, '<Button-1>', lambda e: self.enroll())
        self.enroll_canvas.tag_bind(self.enroll_circle, '<Enter>', self.on_enroll_hover)
        self.enroll_canvas.tag_bind(self.enroll_text, '<Enter>', self.on_enroll_hover)
        self.enroll_canvas.tag_bind(self.enroll_circle, '<Leave>', self.on_enroll_leave)
        self.enroll_canvas.tag_bind(self.enroll_text, '<Leave>', self.on_enroll_leave)
        
        # Make cursor change to hand on hover
        self.enroll_canvas.tag_bind(self.enroll_circle, '<Enter>', 
                                   lambda e: self.enroll_canvas.config(cursor='hand2'))
        self.enroll_canvas.tag_bind(self.enroll_text, '<Enter>', 
                                   lambda e: self.enroll_canvas.config(cursor='hand2'))
        self.enroll_canvas.tag_bind(self.enroll_circle, '<Leave>', 
                                   lambda e: self.enroll_canvas.config(cursor=''))
        self.enroll_canvas.tag_bind(self.enroll_text, '<Leave>', 
                                   lambda e: self.enroll_canvas.config(cursor=''))
    ...
    def on_enroll_hover(self, event):
        """Handle hover effect for Enroll button"""
        self.enroll_canvas.itemconfig(self.enroll_circle, fill='#1a86e3')  # Lighter blue on hover
    
    def on_enroll_leave(self, event):
        """Handle hover leave effect for Enroll button"""
        self.enroll_canvas.itemconfig(self.enroll_circle, fill='#0078d4')  # Back to original blue
    ...
    def enroll(self):
        self.controller.reset()
        # Use custom dialog to get all user information
    ...
    def show_all_users(self):
        """Show all users in a popup window"""
        # Create a new window
