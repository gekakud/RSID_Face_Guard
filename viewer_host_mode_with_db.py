...
# Set to True for RPi5 with small 800x480 screen (WINDOWED mode, NOT fullscreen)
...
def place_window_on_800x480_display(win: tk.Tk, prefer_w=800, prefer_h=440) -> None:
    """
    נסיון "לדחוף" את החלון למסך 800x480 אם יש שני מסכים.
    עובד תחת X11/XWayland עם xrandr. אם אין xrandr/Wayland מלא - פשוט לא עושה כלום.
    """
    try:
        out = subprocess.check_output(["xrandr"], text=True)
        pat = re.compile(r"^(?P<name>\S+)\s+connected\s+(?P<w>\d+)x(?P<h>\d+)\+(?P<x>\d+)\+(?P<y>\d+)")
...
class EnrollDialog(tk.Toplevel):
    ...
    def ok_pressed(self):
        ...
        self.result = {'id': user_id, 'name': name, 'permission_level': permission}
        self.destroy()
...
class GUI(tk.Tk):
    def __init__(self, controller: Controller):
        ...
        if RUN_SMALL_SCREEN:
            ...
            # מיקום המסך הקטן לפי xrandr: +1920+0
            small_screen_x = 1920
            small_screen_y = 0

            self.geometry(f"{max_w}x{max_h}+{small_screen_x}+{small_screen_y}")

            # חלון רגיל (לא fullscreen)
            self.resizable(False, False)

            # להסתיר סמן (אופציונלי למוצר)
            self.config(cursor="none")

        else:
            max_w = int(1280 / 1.5)
            max_h = int(720 / 1.5) + 80
        ...
        # ✅ חשוב: שורת הוידאו תגדל, שורת הכפתורים לא "תבלע" גובה
        self.grid_columnconfigure((0, 1), weight=1)
        self.grid_rowconfigure(0, weight=1)   # video
        self.grid_rowconfigure(1, weight=0)   # buttons

        # Video frame (פחות padding למסך קטן כדי לנצל גובה)
        video_pady = (0, 4) if RUN_SMALL_SCREEN else (0, 20)
        self.video_frame = ttk.Frame(self)
        self.video_frame.grid(row=0, column=0, padx=0, pady=video_pady, sticky="nsew", columnspan=2)
        ...
        # Button frame
        button_pady = (0, 0) if RUN_SMALL_SCREEN else (0, 5)
        self.button_frame = ttk.Frame(self)
        self.button_frame.grid(row=1, column=0, padx=(5, 5), pady=button_pady, sticky="ew", columnspan=2)
        ...
        if RUN_IN_PRODUCTION:
            ...
            self.auth_button.grid(row=0, column=1, padx=(20, 20), pady=(6, 6), ipady=14, sticky="ew")
        else:
            self.auth_button = ttk.Button(self.button_frame, text="Authenticate", command=self.authenticate)
    ...
    def update_video(self):
        ...
        if self.image is not None and canvas_w > 0 and canvas_h > 0:
            ...
            self.canvas.moveto(
                ...
                int((canvas_w - self.scaled_image.width()) / 2),
            ...
            if new_snapshot is not None:
                thumb = ImageOps.contain(
                    new_snapshot, size=(int(canvas_w / 4), int(canvas_h / 4))
                ).transpose(Image.Transpose.FLIP_LEFT_RIGHT)

                self.snapshot_image = ImageTk.PhotoImage(image=thumb)
